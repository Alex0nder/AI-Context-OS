...
export function judgeSystemPrompt(projectName) {
  return `You are an evaluation judge for ${projectName} technical Q&A.
...
export async function judgeAnswer({ question, gold, answer, projectName = "project" }) {
...
${answer}`;
  ...
  const result = await chatCompletion({
    model: judgeModel,
    messages: [
      { role: "system", content: judgeSystemPrompt(projectName) },
