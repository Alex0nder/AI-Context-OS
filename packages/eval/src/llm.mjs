...
export async function chatCompletion(opts) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      await sleep(Math.min(60_000, attempt * 15_000));
    }
  }
  throw lastErr;
}

export function answerSystemPrompt(projectName) {
  return `You answer questions about the ${projectName} project using ONLY the context provided.
...
export async function answerQuestion(question, contextText, condition, projectName = "project") {
  ...
  return chatCompletion({
    model,
    messages: [
      { role: "system", content: answerSystemPrompt(projectName) },
