...
1. **Client Request**: The client sends an HTTP request with the `Authorization: Bearer <token>` header.
...
- **Token Prefix Check**:
  - If the token starts with `mat_`, it calls `resolveMcpAccessToken()` to handle the token as a JWT or fallback to KV.
  - If the token is a SHA-256 hash, it performs a lookup in the `api_keys` table joined with the `teams` table.
  - If the token is a plaintext match, it checks against allowed API keys from the environment, assigning it to the `legacy` plan with `FULL_ACCESS_SCOPE`.
- **Return Value**: 
  - If a valid authentication is found, it returns a `ResolvedAuth` object containing:
    - `hint`: First 16 hex of the SHA-256 token for rate limiting and inbox association.
...
- The `applyAuthContext(c, auth)` function is then called to inject the resolved authentication details into the Hono context, which includes:
  - `apiKeyHint`, `apiPlan`, `teamId`, `apiKeyId`, `apiKeyScope`, and plan limits.

### Rate Limiting
- The rate limiting middleware uses the `hint` from the `ResolvedAuth` to track and limit requests based on the plan's rate limits.
