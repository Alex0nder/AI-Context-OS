...
### Extraction Process
1. **Combine Text and HTML**:
   - Combines `text` and `html` from the Resend receiving API.
...
### Storage
- Extracted OTP and links are stored in `messages.otp` and `messages.links_json` during the `insertMessage()` process in the queue consumer.

### Processing Context
- OTP extraction occurs only during queue processing for reliability, not in the webhook.
